...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    socket.on("admin:alerta-gps", (data) => {
      ...
      setGpsAlerts(prev => [...prev, { ...data, id: alertId }]);
      setTimeout(() => {
        setGpsAlerts(prev => prev.filter(a => a.id !== alertId));
      }, 8000);
    });
            ...
            <div className="lg:col-span-1 bg-zinc-900/40 border border-zinc-900 p-8 rounded-[40px] h-fit">
              ...
              <form onSubmit={handleProductSubmit} className="space-y-4">
                ...
                <textarea 
                  ...
                  className="w-full bg-black border border-zinc-800 rounded-2xl p-4 text-[10px] font-bold h-24 resize-none outline-none focus:border-green-500" 
                />
                <button type="submit" className="w-full bg-white text-black font-black py-5 rounded-2xl uppercase text-[12px] hover:bg-green-500 transition-all active:scale-95">
                  Publicar en Tienda
                </button>
              </form>
            </div>
            ...
            <div className="lg:col-span-2 bg-zinc-900/10 rounded-[40px] border border-zinc-900 p-8">
              ...
              {products.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 gap-4 text-zinc-800">
                  <FiPackage size={48} />
                  <p className="italic font-bold uppercase tracking-widest text-xs">No hay productos publicados...</p>
                </div>
              ) : (
